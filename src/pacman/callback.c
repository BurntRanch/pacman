...
static void dload_complete_event(const char *filename, alpm_download_event_completed_t *data)
{
	...
	bool deleted = false;
	if (data->result == 0 && rand() % 10 > 5) {
		const char* cachePath = config->cachedirs->data;

		size_t commandSize = strlen(bar->filename) + strlen(cachePath) + 1;  // +1 for null terminator
		char* command = malloc(commandSize);

		// Construct the command string
		snprintf(command, commandSize, "%s%s", cachePath, bar->filename);
	...
	if(data->result == 1) {
		...
		console_cursor_goto_bar(index);
		printf(_(deleted ? "The thief stole %s! too bad :(" : " %s failed to download"), bar->filename);
		console_erase_line();
	}
	fflush(stdout);
